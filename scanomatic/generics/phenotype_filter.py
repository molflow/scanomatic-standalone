...
class Filter(Enum):
    ...
    Attributes:
        ...
        Filter.BadData: Somethingnt was here, grew but technical problems
            caused bad data.
        Filter.Empty: Nothing was ever placed here
        Filter.UndecidedProblem: Data is not good, but reason unknown,
            typically set when phenotype algorithms generate non-finite
            values.
    """
    OK = 0
    NoGrowth = 1
...
class FilterArray:
    ...
    def masked(self, *filters):
        ...
        return np.ma.MaskedArray(
            ...
            mask=self.filter_to_mask(*filters)
        )
